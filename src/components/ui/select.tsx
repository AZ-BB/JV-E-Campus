...
>(({ className, children, label, required, ...props }, ref) => (
    <div className="flex flex-col gap-2">
        ...
        <Select.Trigger
            ref={ref}
            className={cn(
                "flex h-7 w-full items-center justify-between rounded-md border border-gray-700 bg-gray-800 px-3 py-2 text-sm ring-offset-white placeholder:text-gray-500 focus:outline-none focus:ring-1 disabled:cursor-not-allowed disabled:opacity-50",
                className
            )}
            {...props}
        >
...
>(({ className, ...props }, ref) => (
    <Select.ItemIndicator ref={ref} className={cn("flex h-3.5 w-3.5 items-center justify-center", className)} {...props}>
        <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
